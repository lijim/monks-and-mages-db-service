...
export const initializeGameResultsEndpoints = (
  ...
  server.post(
    '/game_results',
    async (
      req: Request<EmptyObj, GameResult | ErrorMessage, CreateGameResultsBody>,
      ...
      if (!usernames || !Array.isArray(usernames))
        return res.status(400).send({ message: 'Need a usernames array' });
      if (!winningUsers || !Array.isArray(winningUsers))
        return res.status(400).send({ message: 'Need a winning users array' });
      if (!winningGuests || !Array.isArray(winningGuests))
        return res.status(400).send({ message: 'Need a winning guests array' });
      ...
      try {
        let gameResultInput: Prisma.GameResultCreateInput = {
          guests,
          winningGuests,
          userPlayers: {
